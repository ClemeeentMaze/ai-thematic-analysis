...
function ResponseCard({ response, blockType, hasHighlight = false, isOpenQuestion = false, onNavigateToParticipant, generatedThemes = [] }) {
  ...
  return (
    <div 
      ref={cardRef}
      className="relative py-4 border-b border-[rgba(108,113,140,0.08)] hover:bg-[#FAFBFC] transition-colors"
    >
      {/* Minimal layout: Participant avatar/info on left, transcript, video on right */}
      <Flex gap="MD" alignItems="flex-start">
        {/* Left: Participant indicator */}
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[#E8F4FF] flex items-center justify-center">
          <Text className="text-[#0568FD] font-medium text-sm">
            P{response.participantId.slice(-2)}
          </Text>
        </div>

        {/* Middle: Transcript/Response text */}
        <div className="flex-1 min-w-0">
          {/* Participant link and timestamp - top line */}
          <Flex alignItems="center" gap="SM" className="mb-1">
            <button 
              className="text-[#0568FD] font-medium text-sm hover:underline cursor-pointer"
              onClick={onNavigateToParticipant}
            >
              Participant {response.participantId}
            </button>
            <Text color="default.main.secondary" className="text-xs">•</Text>
          ...
          <div 
            className={`text-neutral-800 leading-relaxed text-[15px] ${isOpenQuestion ? 'cursor-text select-text selectable-text' : ''}`}
          ...
          {isOpenQuestion && (
            <Text color="default.main.secondary" className="text-xs mt-2 italic opacity-60">
              Select text to create highlight
            </Text>
          )}
        </div>
        
        {/* Right: Video thumbnail (if exists) + actions */}
        <Flex flexDirection="column" alignItems="flex-end" gap="SM" className="flex-shrink-0">
          {hasVideo && (
