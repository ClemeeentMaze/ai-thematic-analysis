...
/**
 * Expandable Response Row - Table row that expands inline to show full transcript
 * Collapsed: Shows truncated preview with expand chevron
 * Expanded: Shows full transcript with text selection for highlighting
 */
function ExpandableResponseRow({ response, hasHighlight = false, onNavigateToParticipant, generatedThemes = [] }) {
  ...
  const getPreviewText = () => {
    ...
    return displayText.length > 80 ? displayText.slice(0, 80) + '...' : displayText;
  };

  // Render full transcript text with highlighting support
  const renderFullTranscript = () => {
    ...
    return paragraphs.map((paragraph, pIndex) => {
      ...
      if (activeSelection && paragraph.includes(activeSelection)) {
        ...
        content = paragraph.split(/(\s+)/).map((part, i) => part.match(/^\s+$/) ? part : renderSelectableWord(part, i));
      }
  ...
  return (
    <div ref={rowRef} className="border-b border-[rgba(108,113,140,0.12)] relative">
      {/* Collapsed Row */}
      <div 
        className={`flex items-center py-4 hover:bg-neutral-50 cursor-pointer transition-colors ${isExpanded ? 'bg-[#FAFBFC]' : ''}`}
        onClick={() => setIsExpanded(!isExpanded)}
      >
        {/* Expand/Collapse indicator */}
        <div className="w-[40px] px-3 flex justify-center">
          <ChevronRight 
            size={16} 
            className={`text-[#6C718C] transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`}
          />
        </div>
        <div className="w-[100px] px-3 relative">
          <VideoThumbnail duration={response.clipDuration} />
        </div>
        ...
        <div className="flex-1 px-3">
          <Text className="text-neutral-700 truncate">{getPreviewText()}</Text>
        </div>
      ...
      {/* Expanded Content */}
      {isExpanded && (
        <div className="bg-[#FAFBFC] px-6 pb-4 pt-2 ml-[40px] border-l-2 border-[#0568FD]">
          <Flex gap="LG" alignItems="flex-start">
            {/* Full transcript - selectable */}
            <div 
              className="flex-1 cursor-text select-text selectable-text"
              onMouseUp={handleMouseUp}
            >
              <Text color="default.main.secondary" className="text-xs font-medium uppercase tracking-wide mb-2">
                Full Transcript
              </Text>
              <div className="text-neutral-800 leading-relaxed">
                {renderFullTranscript()}
              </div>
              <Text color="default.main.secondary" className="text-xs mt-3 italic opacity-60">
            ...
            <div className="flex-shrink-0">
              <button 
                className="text-[#0568FD] text-sm hover:underline cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
    ...
    <ScrollContainer className="h-full">
      <Flex flexDirection="column" className="p-6">
        ...
        <div>
          ...
          {activeTab === 'all' && (
            <>
              {/* Use expandable rows for Open Question / AI Conversation blocks */}
              {(block.type === 'input' || block.type === 'ai_conversation') ? (
                <>
                  {/* Table header for expandable rows */}
                  <div className="flex items-center py-3 bg-[#F8F8FB] rounded-t-lg text-xs font-semibold text-[#6C718C] uppercase tracking-wide">
                    <div className="w-[40px] px-3"></div>
                    <div className="w-[100px] px-3">CLIPS</div>
                    <div className="w-[140px] px-3">PARTICIPANT</div>
                    <div className="flex-1 px-3">RESPONSE PREVIEW</div>
                    <div className="w-[160px] px-3">RESPONDED AT</div>
